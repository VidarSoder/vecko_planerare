    ...
    return (
        <AlertDialog open={isOpen} onOpenChange={onClose}>
            <AlertDialogContent className="max-w-5xl p-8">
                <AlertDialogHeader>
                    <AlertDialogTitle style={{ color: textColor }}>Ladda Schema</AlertDialogTitle>
                    <AlertDialogDescription style={{ color: textColor }}>
                        Välj ett schema att ladda.
                    </AlertDialogDescription>
                </AlertDialogHeader>
                ...
                <div className="overflow-y-auto max-h-96 p-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                        {sortedBlobs.map((blob) => (
                            <div
                                key={blob.id}
                                className={`p-6 border rounded-lg shadow-md cursor-pointer transition-transform transform hover:scale-105 ${selectedSchedule === blob.id ? 'border-blue-500 bg-blue-100' : 'border-gray-300'}`}
                ...
                <AlertDialogFooter>
                    ...
                    <AlertDialogAction onClick={() => selectedSchedule && onConfirm(selectedSchedule)}>Confirm</AlertDialogAction>
                </AlertDialogFooter>
            </AlertDialogContent>
        </AlertDialog>
    );
